...
const Body = ({ movies, tv, isLogged }: { movies: MovieData[], tv: TvData[], isLogged: boolean }) => {
    ...
    <section className="relative flex-1 flex justify-center h-[93.5vh] mt-[200px]">
      <div className="w-[90%] mt-40 flex flex-col gap-8 ">
        <div className="flex flex-col gap-5 ">
          ...
          <CarouselWrapper contentList={movies} isLogged={isLogged} />
        </div>
        <div className="flex flex-col gap-5 ">
          <p className="text-xl font-bold ">Trending Tv Shows</p>
          <CarouselWrapper contentList={tv} isLogged={isLogged} />
        </div>
      </div>
    </section >
  );
